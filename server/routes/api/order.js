...
router.get('/',auth,  async (req, res) => {
  try {
    ...
    if(req.user.role == "ROLE_ADMIN"){
      filter = (all == "1") ? {user: { $ne: user }} : {user};
    }
    else if (req.user.role == "ROLE_MERCHANT"){
      filter = {};
    }
    else if (req.user.role == "ROLE_MEMBER"){
      filter = {user};
    }
    // let filter = (all == "1") ? () ? {user: { $ne: user }} : {} : {user}
    let ordersDoc = await Order.find(filter).populate({
      path: 'cart',
      populate: {
        path: 'products.product',
        populate: {
      ...
      if(req.user.role == "ROLE_MERCHANT"){
        orders = orders.filter(order => {
          if(order.products && order.products.length > 0){
            let merchant_products = order.products.filter(product => {
              return product.product?.brand.merchant.toString() == req.user.merchant.toString()
            });
            if(merchant_products.length > 0) return true;
          }
          return false;
        })
      }
